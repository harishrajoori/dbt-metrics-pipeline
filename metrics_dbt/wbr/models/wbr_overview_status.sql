...
WITH
    ...
    (
        SELECT
            'acquisition_mart' as metric_mart_name, date_scope,financial_year,week_number,year_month_day,week_commencing_date,product_pl,region,
            category,
            sub_category,
        ...
        WHERE 
            metric_name IN 
            (
                'Fee Revenue / Transaction','Fee Revenue','Non-Fee Revenue / Visit','Non-Fee Revenue'
            )
        UNION ALL
         SELECT
         ...
         WHERE
             metric_name IN
             (
                 'Avg Trans Value Budget','Avg Trans Value','Conversion Ratio',
                 'Gross Transactions Budget','WBR Gross Transactions','Total Visits','Transactions Tracked'
             )
         UNION ALL
         SELECT
    ...
    (
        ...
        FROM
            (
                ...
                WHERE
                    1=1 -- Same filters used in the base tables so commenting out these filters
{#                    (#}
{#                        (#}
{#                            product_pl = 'EU'#}
{#                            AND#}
...
{#                    )#}
                    AND (
                    (metric_mart_name = 'retention_mart'
                        AND metric_name NOT IN ('Total number of Orders',
                                               'Net Sales Amount',
                                               'Number of Customers',
                                               'Transactions per Customer'
                                               )
