{%- macro ads_revenue() -%}
/* -- Script Name = 101_ads_revenue.sql :: BEGIN -- */
-- We don't have P&L for Ads, we only post it to UK and region is null
SELECT DISTINCT
    year_month_day
    ...
    , ROUND(SUM(CAST(event_total_line_item_level_cpm_and_cpc_revenue AS DOUBLE)/1000000), 4) AS metric_value
FROM (
    SELECT
...
/* -- Script Name = 101_ads_revenue.sql :: END -- */
{%- endmacro -%}