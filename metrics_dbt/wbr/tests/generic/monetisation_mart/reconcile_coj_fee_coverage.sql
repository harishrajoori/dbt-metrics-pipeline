{% test reconcile_coj_fee_coverage(model) %}

with coj_fee_coverage as (
    SELECT
...
, total_coj_fee_coverage as (
    select
        year_month_day
        , product_pl
        , region
        , ROUND(SUM(CAST(coj_fee_number_of_tickets AS DOUBLE)) / SUM(CAST(total_number_of_tickets AS DOUBLE)), 4) AS metric_value
    from coj_fee_coverage
    group by year_month_day, product_pl, region
)

, monetisation_mart_coj_fee_coverage as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_coj_fee_coverage fmp
    INNER JOIN monetisation_mart_coj_fee_coverage mm
