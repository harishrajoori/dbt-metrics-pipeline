{% test reconcile_refund_fee_revenue(model) %}

with refund_fee_revenue as (
    SELECT
    ...
    WHERE
        ...
        AND NOT (product_pl_code = 'EU' AND DATE(year_month_day) BETWEEN DATE('2023-01-01') AND DATE('2023-01-31'))
)

, total_refund_fee_revenue as (
    select
        year_month_day
        , product_pl
        , region
        , ROUND(IF(product_pl = 'UK' OR product_pl IS NULL, SUM(refund_fee_revenue_gbp), SUM(refund_fee_revenue_eur)), 4) AS metric_value
    from refund_fee_revenue
    group by year_month_day, product_pl, region
)

, monetisation_mart_refund_fee_revenue as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_refund_fee_revenue fmp
    INNER JOIN monetisation_mart_refund_fee_revenue mm
