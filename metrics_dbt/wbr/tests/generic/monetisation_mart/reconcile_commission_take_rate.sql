{% test reconcile_commission_take_rate(model) %}

with commission_take_rate as (
    SELECT
...
, total_commission_take_rate as (
    ...
    from commission_take_rate
    group by year_month_day, product_pl, region
)

, monetisation_mart_commission_take_rate as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_commission_take_rate fmp
    INNER JOIN monetisation_mart_commission_take_rate mm
