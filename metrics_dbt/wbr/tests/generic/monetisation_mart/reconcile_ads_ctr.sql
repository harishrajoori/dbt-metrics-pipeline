...
with gam_ads_ctr as (
    SELECT
        ...
        , event_total_line_item_level_clicks
...
, total_gam_ads_ctr as (
    select
        year_month_day
        , ROUND(SUM(CAST(event_total_line_item_level_clicks AS DOUBLE)) / SUM(cast(event_total_line_item_level_impressions AS DOUBLE)), 4) AS metric_value
    from gam_ads_ctr
    where rn = 1
    group by year_month_day
)

, monetisation_mart_ads_ctr as (
    select
        year_month_day
        , category
        , sub_category
...
, compare_cte AS (
    ...
    FROM total_gam_ads_ctr gam
    INNER JOIN monetisation_mart_ads_ctr mm
