{% test reconcile_refund_fee_take_rate(model) %}

with refund_fee_take_rate as (
    SELECT
...
, total_refund_fee_take_rate as (
    ...
    from refund_fee_take_rate
    group by year_month_day, product_pl, region
)

, monetisation_mart_refund_fee_take_rate as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_refund_fee_take_rate fmp
    INNER JOIN monetisation_mart_refund_fee_take_rate mm
