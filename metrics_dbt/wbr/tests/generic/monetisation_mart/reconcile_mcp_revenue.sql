{% test reconcile_mcp_revenue(model) %}
...
, total_mcp_revenue as (
    select
        year_month_day
        , product_pl
        , region
        , ROUND(IF(product_pl = 'UK' OR product_pl IS NULL, SUM(mcp_revenue_gbp), SUM(mcp_revenue_eur)), 4) AS metric_value
    from mcp_revenue
    group by year_month_day, product_pl, region
)

, monetisation_mart_mcp_revenue as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_mcp_revenue fmp
    INNER JOIN monetisation_mart_mcp_revenue mm
