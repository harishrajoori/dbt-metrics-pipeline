{% test reconcile_booking_fee_take_rate(model) %}

with booking_fee_take_rate as (
    SELECT
...
, total_booking_fee_take_rate as (
    select
        year_month_day
        , product_pl
        , region
        , IF( product_pl = 'UK' OR product_pl IS NULL,
            ROUND(SUM(CAST(booking_fee_revenue_gbp AS DOUBLE)) / SUM(CAST(net_sales_amount_gbp AS DOUBLE)), 4),
            ROUND(SUM(CAST(booking_fee_revenue_eur AS DOUBLE)) / SUM(CAST(net_sales_amount_eur AS DOUBLE)), 4)
        ) AS metric_value
    from booking_fee_take_rate
    group by year_month_day, product_pl, region
)

, monetisation_mart_booking_fee_take_rate as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_booking_fee_take_rate fmp
    INNER JOIN monetisation_mart_booking_fee_take_rate mm
