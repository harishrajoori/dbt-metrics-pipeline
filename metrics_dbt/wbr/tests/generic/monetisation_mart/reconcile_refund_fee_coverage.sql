{% test reconcile_refund_fee_coverage(model) %}

with refund_fee_coverage as (
    SELECT
...
, total_refund_fee_coverage as (
    select
        year_month_day
        , product_pl
        , region
        , ROUND(SUM(CAST(refund_fee_number_of_tickets AS DOUBLE)) / SUM(CAST(total_number_of_tickets AS DOUBLE)), 4) AS metric_value
    from refund_fee_coverage
    group by year_month_day, product_pl, region
)

, monetisation_mart_refund_fee_coverage as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_refund_fee_coverage fmp
    INNER JOIN monetisation_mart_refund_fee_coverage mm
