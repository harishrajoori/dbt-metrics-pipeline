{% test reconcile_booking_fee_revenue(model) %}

{{ config(severity = 'error') }}

with booking_fee_revenue as (
    SELECT
        ...
        END AS region
        , m_revenue_booking_fee_eur AS booking_fee_eur
        , m_revenue_booking_fee_gbp AS booking_fee_gbp
...
, total_booking_fee_revenue as (
    select
        year_month_day
        , product_pl
        , region
        , IF(product_pl = 'UK' OR product_pl IS NULL, ROUND(SUM(booking_fee_gbp), 4), ROUND(SUM(booking_fee_eur), 4)) AS metric_value
    from booking_fee_revenue
    group by year_month_day, product_pl, region
)

, monetisation_mart_booking_fee_revenue as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_booking_fee_revenue fmp
    INNER JOIN monetisation_mart_booking_fee_revenue mm
