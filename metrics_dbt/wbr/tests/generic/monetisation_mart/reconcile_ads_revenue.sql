{% test reconcile_ads_revenue(model) %}

with gam_ads_revenue as (
    SELECT
        DATE(event_date) AS year_month_day
        , 'UK' AS product_pl
        , 'United Kingdom' AS region
        , event_total_line_item_level_cpm_and_cpc_revenue as revenue
...
, total_gam_ads_revenue as (
    select
        year_month_day
        , ROUND(SUM(CAST(revenue AS DOUBLE)/1000000), 4) AS metric_value
    from gam_ads_revenue
    where rn = 1
    group by year_month_day
)

, monetisation_mart_ads_revenue as (
    select
        year_month_day
        , category
        , sub_category
...
, compare_cte AS (
    ...
    FROM total_gam_ads_revenue gam
    INNER JOIN monetisation_mart_ads_revenue mm
