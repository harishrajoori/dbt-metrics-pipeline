{% test reconcile_booking_fee_coverage(model) %}

with booking_fee_coverage as (
    SELECT
...
, total_booking_fee_coverage as (
    select
        year_month_day
        , product_pl
        , region
        , ROUND(SUM(CAST(booking_fee_number_of_tickets AS DOUBLE)) / SUM(CAST(total_number_of_tickets AS DOUBLE)), 4) AS metric_value
    from booking_fee_coverage
    group by year_month_day, product_pl, region
)

, monetisation_mart_booking_fee_coverage as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_booking_fee_coverage fmp
    INNER JOIN monetisation_mart_booking_fee_coverage mm
