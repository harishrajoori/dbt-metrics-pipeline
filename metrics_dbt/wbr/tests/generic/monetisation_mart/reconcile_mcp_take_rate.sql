{% test reconcile_mcp_take_rate(model) %}

with mcp_take_rate as (
    SELECT
...
, monetisation_mart_mcp_take_rate as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_mcp_take_rate fmp
    INNER JOIN monetisation_mart_mcp_take_rate mm
