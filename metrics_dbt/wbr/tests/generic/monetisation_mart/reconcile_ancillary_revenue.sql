{% test reconcile_ancillary_revenue(model) %}
...
, total_ancillary_revenue as (
    select
        year_month_day
        , product_pl
        , region
        , ROUND(IF(product_pl = 'UK' OR product_pl IS NULL, SUM(insurance_commission_revenue_gbp), SUM(insurance_commission_revenue_eur)), 4) AS metric_value
    from ancillary_revenue
    group by year_month_day, product_pl, region
)

, monetisation_mart_ancillary_revenue as (
    select
        year_month_day
        , product_pl
        , region
...
, compare_cte AS (
    ...
    FROM total_ancillary_revenue fmp
    INNER JOIN monetisation_mart_ancillary_revenue mm
