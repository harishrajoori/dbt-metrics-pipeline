{% test reconcile_user_journey_leisure(model) %}
...
(
    ...
    and    date_scope = 'DAY'
    and    sub_category = 'Leisure and Long Distance'
    group by 1,2,3
),
...
where year_month_day BETWEEN ('{{ env_var("exe_start_date") }}') AND ('{{ env_var("exe_end_date") }}')
    ...
    AND fare_train_classification_name_longest_duration = 'long distance'
    AND product_travel_classification_name NOT IN ('Commuting','Business')
group by 1,2,3
)
