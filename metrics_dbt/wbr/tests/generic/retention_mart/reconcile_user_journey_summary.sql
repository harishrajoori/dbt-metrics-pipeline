{% test reconcile_user_journey_summary(model) %}
...
where year_month_day BETWEEN ('{{ env_var("exe_start_date") }}') AND ('{{ env_var("exe_end_date") }}')
    ...
    AND ((fare_train_classification_name_longest_duration = 'long distance' AND product_travel_classification_name NOT IN ('Commuting','Business'))
        OR (fare_train_classification_name_longest_duration = 'regional' AND product_travel_classification_name = 'Commuting'))
group by 1,2,3
)
...
where  retention.net_sales_amount <> fm_data.net_sales_amount
{% else %}
