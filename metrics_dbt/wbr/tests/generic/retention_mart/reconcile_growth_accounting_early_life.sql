{% test reconcile_growth_accounting_early_life(model) %}
    ...
    select 
           ...
           , CASE upper('{{ env_var("exe_type") }}')
              ...
              END AS year_month_day
                 --Early Life--
           , sum(el_volume) as customer_count
           , round(IF((product_pl = 'UK' OR product_pl IS NULL), sum(el_nts_gbp), sum(el_nts_eur)), 4) AS net_sales_amount
           , sum(el_retention) as retention_count
           , sum(el_churned) as churned_count
