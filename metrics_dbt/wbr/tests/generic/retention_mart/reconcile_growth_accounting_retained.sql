{% test reconcile_growth_accounting_retained(model) %}
    ...
    select 
           ...
           , CASE upper('{{ env_var("exe_type") }}')
              ...
              END AS year_month_day
                 --Retained--
           , sum(ret_volume) as customer_count
           , round(IF((product_pl = 'UK' OR product_pl IS NULL), sum(ret_nts_gbp), sum(ret_nts_eur)), 4) AS net_sales_amount
           , sum(ret_retention) as retention_count
           , sum(ret_churned) as churned_count
