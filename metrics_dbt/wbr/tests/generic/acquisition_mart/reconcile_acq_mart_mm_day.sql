{% test reconcile_acq_mart_mm_day(model) %}
WITH am AS (
...
mm AS (
    SELECT date(activity_date) as year_month_day
        ,CASE
           WHEN kpi IN ('Incremental New Customers', 'Incremental Spend EUR') THEN 'Paid Incremental'
...
WHERE (CAST(metric_value AS bigint) <> CAST(value AS bigint)
